...
$( document ).ready(function() {
    render_chart(json_data = "data/sirohi_tool_timesp_monthly.json");
 });
...
layer.selectAll("text")
       ...
       .append("text")
        .text(function(d) {
           if(!(d.y0 == 0 && d.y1 == 0.25) && (d.y1 > 20)){
             return Math.floor(d.y0) + " to " + Math.floor(d.y1) }
           })
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("Average Time Spent(minutes) in a Day by Students on Tools (Month wise)");
...
svg.append("text")
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("*MIT tools, some of the Math tools and all English tools are not included");
 ...
 svg.append("text")
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("*By all students in the class both logged-in and anonymous in a day")
...
svg.append("text")
    ...
    d3.selectAll(".m")
				.on("click", function() {
					...
					var str;
					if(date == "Sirohi"){
						str = "data/sirohi_tool_timesp_monthly.json";
					}else if(date == "Jaipur"){
						str = "data/jaipur_tool_timesp_monthly.json";
					}else if(date == "Warangal"){
						str = "data/warangal_tool_timesp_monthly.json";
					}else if(date == "Karimnagar"){
						str = "data/karimnagar_tool_timesp_monthly.json";
					}else{
					//All states top 50 schools
						str = "data/rangareddy_tool_timesp_monthly.json";
					}
