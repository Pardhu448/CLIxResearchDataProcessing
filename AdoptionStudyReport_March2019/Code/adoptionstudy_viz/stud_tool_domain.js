$( document ).ready(function() {
    render_chart(json_data = "data/sirohi_tool_stud_monthly.json");
 });
...
svg.append("text")
      ...
      .text("Number of Students");
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("Range of Number of Students* Engaged with Tools Section of the Platform (Month wise)");
...
svg.append("text")
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("*MIT tools are not considered for analysis, some of the math and all english tools are not included. Also only logged-in students are considered");
    ...
    d3.selectAll(".m")
				.on("click", function() {
					...
					var str;
					if(date == "Sirohi"){
						str = "data/sirohi_tool_stud_monthly.json";
					}else if(date == "Jaipur"){
						str = "data/jaipur_tool_stud_monthly.json";
					}else if(date == "Warangal"){
						str = "data/warangal_tool_stud_monthly.json";
					}else if(date == "Karimnagar"){
						str = "data/karimnagar_tool_stud_monthly.json";
					}else{
					//All states top 50 schools
						str = "data/rangareddy_tool_stud_monthly.json";
					}
 ...
 //   var dataset
    ...
    //    .domain(dataset["categories"])
    //    .rangeRoundBands([0, width], .08);
    y_max_range = yGroupMax
    if (yGroupMax < 5){
      y_max_range = 10
    }

    var y = d3.scale.linear()
        .domain([0, y_max_range])
        .range([height, 0]);
    //console.log(yGroupMin)
    //console.log(yGroupMax)
    var xAxis = d3.svg.axis()
...
svg.append("text")
	...
	//      .remove();
        ...
        .attr("dx", ".71em")
        .attr("dy", "-.71em")
        .text("Range of Number of Students* Engaged with Tools Section of the Platform (Monthwise)");
