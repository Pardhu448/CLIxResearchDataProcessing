...
labusage_data['domain'] = labusage_data['module_name']
labusage_data = labusage_data.replace({'domain': domain_module_map})
domainwise_activity_num = labusage_data.groupby(['domain']).apply(lambda x: get_module_metrics_v1(x)).reset_index()
#modulewise_schools['module_name'] = modulewise_schools['module_name'].apply(lambda x: x.split("'")[1])

#domainwise_activity_num.to_csv(path_dest + '/mz_modules_data_V1.csv')
#domainwise_activity_num.to_csv(path_dest + '/rj_modules_activities_data_V1.csv')
#domainwise_activity_num.to_csv(path_dest + '/tg_modules_activities_data_V1.csv')
#domainwise_activity_num.to_csv(path_dest + '/ct_modules_activities_data_V1.csv')
#domainwise_activity_num.to_csv(path_dest + '/AllStates_domainwise_activities_data.csv')
